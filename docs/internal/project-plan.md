...
- provider-OAuth callback adapter; the app-install service now has a worker-style state-driven callback entry point, a fixture provider exchange port, and a credential-store adapter that writes only non-secret metadata while routing token payloads to a dedicated secret sink;
