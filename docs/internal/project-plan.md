...
- provider-OAuth credential handoff port; the app-install service now has an internal-only provider code exchange seam plus a dedicated credential storage/encryption seam, records successful internal exchanges as `credential_ready`/`provider_oauth`, and keeps provider codes, tokens, raw provider responses, credential refs, account refs, and account labels out of app-install tables and public app-install read models;
- provider-OAuth callback adapter; the app-install service now has a worker-style state-driven callback entry point, a fixture provider exchange port, a credential-store adapter that writes only non-secret metadata while routing token payloads to a sealed Postgres token-secret table, and a live Compose-Postgres smoke covering the `credential_ready`/`provider_oauth` callback path plus historical check-constraint compatibility;
