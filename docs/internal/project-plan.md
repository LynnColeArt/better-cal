...
- app metadata reader or generated app catalog; the first canary now persists non-secret integration app metadata in `integration_app_catalog`, exposes `GET /v2/apps` behind `policy.apps.read`, and returns provider/category/auth/capability metadata without credential refs, account refs, provider tokens, raw provider responses, or signing material;
