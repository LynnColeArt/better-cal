...
- app install intent storage and `POST /v2/app-install-intents` persist only user id, selected app slug, opaque intent refs, pending status, and timestamps, and responses never include provider credentials, account refs, provider tokens, raw provider responses, provider error bodies, or signing material;
