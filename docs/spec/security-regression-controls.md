...
- booking selected/destination calendar refs and opaque external calendar event ids remain internal booking state and provider payload hints only; they are not exposed in public booking DTOs;
- app catalog storage and `GET /v2/apps` expose only non-secret category, provider, name, description, auth type, capability, and timestamp fields, never credential refs, account refs, provider tokens, raw provider responses, provider error bodies, or signing material;
- app install intent storage, installed-app projection storage, `POST /v2/app-install-intents`, `GET /v2/app-install-intents`, `GET /v2/app-installations`, `POST /v2/app-install-intents/{installIntentRef}/external-auth`, `GET /v2/app-install-intents/{installIntentRef}/external-auth`, `POST /v2/app-install-intents/{installIntentRef}/external-auth/consume`, `POST /v2/app-install-intents/{installIntentRef}/external-auth/authorization-preview`, `POST /v2/app-install-intents/{installIntentRef}/external-auth/callback-preflight`, `POST /v2/app-install-intents/{installIntentRef}/external-auth/provider-exchange`, `POST /v2/app-install-intents/{installIntentRef}/complete`, `POST /v2/app-install-intents/{installIntentRef}/activate`, and `GET /v2/app-install-intents/{installIntentRef}/progress` persist only user id, selected app slug, opaque intent refs, pending, `requires_external_auth`, installed, blocked, or active status, one-time handoff state refs, authorization preview refs, callback preflight refs, provider exchange refs, install completion refs, app installation refs, provider-neutral preview/callback/exchange/completion/activation status, simulated exchange/completion/activation mode, expiry timestamps, consumed timestamps, requested/received/recorded/completed/activated timestamps, and audit timestamps, scope reads and status transitions to the current user, derive external-auth descriptors, authorization previews, callback preflights, simulated provider exchanges, simulated completions, simulated app-installation activations, app-installation lists, and progress reads only from owned handoff-ready intents plus non-secret app metadata, reject expired or replayed handoff state, reject provider code fields in callback preflight, provider-exchange, completion, and activation payloads, and responses never include provider credentials, account refs, provider tokens, redirect URLs, callback URLs, raw callback query strings, provider authorization codes, raw provider responses, provider error bodies, or signing material;
- calendar connection and calendar catalog routes expose only opaque refs plus non-secret provider metadata, account refs, account email, writable flags, and status, and do not return credential payloads, provider tokens, or signing material;
- calendar catalog sync persists only provider connection/catalog snapshots and generic connection status transitions, never credential payloads, provider access tokens, refresh tokens, raw provider responses, or provider error bodies; status transitions and provider-row pruning happen transactionally with the synced catalog update;
- credential metadata storage and `GET /v2/credentials` expose only opaque refs, app/provider metadata, account refs or labels, status, generic status codes, scopes, and timestamps, never encrypted payloads, provider keys, provider access tokens, refresh tokens, raw provider responses, raw provider error bodies, or signing material;
- provider status refresh stores only generic status, sanitized status codes, and checked timestamps for credential metadata and calendar connections; raw provider errors, provider response bodies, credential payloads, access tokens, and refresh tokens remain outside storage and responses;
- selected/destination calendar management routes expose only opaque refs plus non-secret provider metadata and do not return credential payloads, provider tokens, or signing material;
