...
4. API v2 auth route compatibility for the starter fixture set. Started in `backend/internal/httpapi/` with credential verification factored into `backend/internal/auth/`; the first OAuth token endpoint canary now exchanges a fixture authorization code once, stores only hashed authorization codes and token values in Postgres, denies replay, and lets a valid unexpired access token authenticate `GET /v2/bookings/{bookingUid}` with permissions narrowed to token scopes.
