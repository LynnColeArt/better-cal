...
11. Booking validation service with durable state and side-effect ports. Fixture lifecycle behavior, explicit booking row persistence, and the first persistence canary have started in `backend/internal/booking/`; request validation now rejects unsupported fixture event types, malformed times, invalid attendee contact data, secret-bearing echoed maps, unavailable create slots, and unavailable reschedule slots through the shared slots-service adapter. Cancel, reschedule, confirm, and decline now plan fixture side effects through typed ports that receive minimal booking snapshots rather than raw response or metadata maps. The Postgres repository now commits booking rows, idempotency keys, and planned side-effect rows atomically, with tests for duplicate idempotency replay and rollback on failed side-effect persistence. Planned webhook side effects now persist typed payload hints so retries can rebuild webhook bodies without re-reading request-only inputs, email side effects now persist the payload hints needed for retry-safe email reconstruction, and calendar side effects now persist the payload hints needed for retry-safe reschedule canaries, including selected/destination calendar refs and opaque external event ids. A matching current-user calendar-management slice now lives in `backend/internal/calendars/`, with Postgres-backed selected-calendar rows, destination-calendar pointers, and API routes for reading, adding, deleting, and choosing the destination calendar without exposing provider secrets. The provider-backed catalog slice now syncs Postgres-backed calendar connection rows plus calendar catalog rows from the typed calendar provider adapter, exposes read routes for each, records connection status transitions transactionally, prunes stale provider catalog rows, refreshes selected-calendar snapshots, and validates selected-calendar writes against that synced catalog so calendar refs come from an explicit integration boundary instead of arbitrary user-supplied metadata. The first credential metadata canary in `backend/internal/credentials/` now persists and exposes only non-secret integration refs, app/provider metadata, account labels, status, and scopes through `GET /v2/credentials`, while `backend/internal/integrations/` provides a generic status refresh boundary that updates only sanitized credential and calendar connection status codes plus checked timestamps. The first app catalog canary in `backend/internal/apps` now persists and exposes only non-secret app/category/provider/auth/capability metadata through `GET /v2/apps` without credential refs, account refs, provider tokens, raw provider responses, or signing material, app install intents persist only an opaque intent ref plus pending status through `POST /v2/app-install-intents`, current-user install intents are readable through `GET /v2/app-install-intents`, owned pending intents can move to `requires_external_auth` through `POST /v2/app-install-intents/{installIntentRef}/external-auth`, owned handoff-ready intents can produce a persisted read-only descriptor through `GET /v2/app-install-intents/{installIntentRef}/external-auth` with provider/display/auth/scope metadata plus a non-secret handoff state ref and expiry, that state can be consumed exactly once through `POST /v2/app-install-intents/{installIntentRef}/external-auth/consume`, and the same one-time gate can produce a metadata-only authorization preview through `POST /v2/app-install-intents/{installIntentRef}/external-auth/authorization-preview` with provider slug, requested scopes, state ref, placeholder authorization endpoint id, preview status, and expiry, without OAuth callback, redirect URL generation, provider token, credential exchange plumbing, or provider calls. The first worker canary is packaged as an optional Compose service, claims planned side-effect rows with row locks, dispatches them through durable Postgres dispatch-log, booking-webhook-delivery, booking-email-delivery, and booking-calendar-dispatch adapters, selects active webhook subscribers by trigger, resolves signing secrets by key ref, snapshots signed webhook attempts, snapshots email attempts, snapshots calendar canary attempts, sends real HTTP webhook POSTs, routes email attempts through the first typed provider adapter in `backend/internal/email/`, sends real HTTP email provider POSTs, routes calendar canary attempts through the first typed provider adapter in `backend/internal/calendar/`, sends real HTTP calendar provider POSTs, retries only the pending subscriber rows, records generic email and calendar failure text plus response codes, dead-letters exhausted webhook attempts, disables failing subscriptions, and logs basic delivery metrics while the real provider integrations are still pending.
12. App install callback preflight canary. `backend/internal/apps` now records a provider-neutral callback preflight through `POST /v2/app-install-intents/{installIntentRef}/external-auth/callback-preflight` using only callback preflight ref, state ref, app/provider metadata, callback status, preflight status, received timestamp, and audit timestamps. The route rejects unknown provider-code fields, expired state, wrong-owner state, and preflight replay without capturing raw callback query strings, provider codes, redirect URLs, provider tokens, credential refs, raw provider responses, or provider calls.
13. App install progress read model. `backend/internal/apps` now persists metadata-only authorization preview rows and exposes `GET /v2/app-install-intents/{installIntentRef}/progress` as a current-user read model over intent status, descriptor availability, latest handoff state, authorization preview state, callback preflight state, provider-exchange state, completion state, installed-app projection state, and next app-install action. The progress surface keeps provider codes, raw callback inputs, redirect URLs, provider responses, credential refs, account refs, and provider tokens out of storage and responses.
14. App install provider-exchange stub. `backend/internal/apps` now records a simulated provider exchange through `POST /v2/app-install-intents/{installIntentRef}/external-auth/provider-exchange` using only callback preflight ref, state ref, app/provider metadata, queued or blocked exchange status, simulated exchange mode, recorded timestamp, and audit timestamps. The route rejects unknown provider-code fields, wrong-owner state, missing callback preflight, and exchange replay without provider calls, token exchange, credential persistence, account refs, raw callback inputs, raw provider responses, redirect URLs, or signing material.
15. App install completion boundary. `backend/internal/apps` now records terminal simulated install completion through `POST /v2/app-install-intents/{installIntentRef}/complete` using only provider exchange ref, app/provider metadata, installed or blocked completion status, simulated completion mode, completed timestamp, and audit timestamps. Completion updates the install intent to terminal installed or blocked state and rejects unknown provider-code fields, wrong-owner provider exchange refs, missing provider exchanges, not-ready intents, and completion replay without credential creation, provider account creation, provider calls, token exchange, raw callback inputs, raw provider responses, redirect URLs, or signing material.
16. App installation activation canary. `backend/internal/apps` now records a current-user installed-app projection through `POST /v2/app-install-intents/{installIntentRef}/activate` using only a terminal install completion ref, app/provider catalog metadata, active activation status, simulated activation mode, and timestamps. `GET /v2/app-installations` lists those projections for the current user, and progress reads flip to `active` once the projection exists. Activation rejects unknown provider-code fields, wrong-owner completions, blocked completions, and replay without credential refs, provider accounts, provider calls, token exchange, raw provider responses, redirect URLs, or signing material.
17. Provider-OAuth credential handoff port. `backend/internal/apps` now has an internal-only `ExchangeExternalAuthProviderOAuth` contract that validates owned callback preflight state, sends provider codes to a `ProviderOAuthExchangePort`, sends returned token payloads only to a `ProviderCredentialStore` storage/encryption seam, and records the app-install exchange as `credential_ready` with `provider_oauth` mode. The public app-install models still omit provider codes, provider token payloads, raw provider responses, credential refs, account refs, and account labels, and failure tests prove no app-install exchange is recorded when credential storage rejects the secret payload.
18. Provider-OAuth callback adapter. `backend/internal/apps` now exposes a worker-style `HandleProviderOAuthCallback` entry point that accepts an opaque state ref plus provider authorization code, resolves the callback preflight internally, rejects replay before calling provider/credential side effects, and returns only a sanitized internal callback result. `backend/internal/credentials` now has a credential-store adapter that writes non-secret credential metadata while sending provider token payloads to a sealed Postgres token-secret table, and the API process wires those fixture ports when Postgres is enabled. The Postgres callback-mode smoke now covers the live `credential_ready`/`provider_oauth` path, sealed token persistence, and historical check-constraint compatibility.
