...
Inspect captured Compose webhook and calendar-canary deliveries:
...
Webhook retries use `CALDIY_WEBHOOK_MAX_ATTEMPTS`, which defaults to `3`. When an attempt reaches the threshold, the attempt is dead-lettered and the subscriber row is disabled so future deliveries skip it until an operator re-enables it. Calendar canary delivery uses `CALDIY_CALENDAR_DISPATCH_URL`, which defaults in Compose to the local sink at `http://webhook-sink:8090/caldiy/calendar-dispatch`.
