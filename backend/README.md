...
Inspect captured Compose webhook deliveries:
...
Webhook retries use `CALDIY_WEBHOOK_MAX_ATTEMPTS`, which defaults to `3`. When an attempt reaches the threshold, the attempt is dead-lettered and the subscriber row is disabled so future deliveries skip it until an operator re-enables it.
