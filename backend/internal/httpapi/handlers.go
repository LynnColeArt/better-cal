...
func (s *Server) readAppInstallations(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallationsRead) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	installations, err := s.apps().ReadAppInstallations(r.Context(), principal.ID)
	if err != nil {
		if errors.Is(err, apps.ErrInvalidAppInstallation) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid app installation", true)
			return
		}
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
...
func (s *Server) readAppInstallIntents(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallIntentsRead) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	intents, err := s.apps().ReadInstallIntents(r.Context(), principal.ID)
	if err != nil {
...
func (s *Server) readAppInstallIntentExternalAuth(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallExternalAuthRead) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	descriptor, err := s.apps().ReadExternalAuthDescriptor(r.Context(), principal.ID, r.PathValue("installIntentRef"))
	if err != nil {
		if errors.Is(err, apps.ErrInvalidInstallIntent) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid app install intent", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "App install intent not found", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotReady) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "App install intent is not ready for external auth", true)
			return
		}
		if errors.Is(err, apps.ErrAppNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "App not found", true)
	...
	s.sendJSON(w, r, http.StatusOK, envelope{
		Status: "success",
		Data: map[string]any{
			"externalAuth": descriptor,
			"requestId":    s.requestID(r),
		},
	})
}

func (s *Server) markAppInstallIntentExternalAuth(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallIntentsHandoff) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	intent, err := s.apps().MarkInstallIntentRequiresExternalAuth(r.Context(), principal.ID, r.PathValue("installIntentRef"))
	if err != nil {
		if errors.Is(err, apps.ErrInvalidInstallIntent) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid app install intent", true)
			return
...
func (s *Server) consumeAppInstallIntentExternalAuth(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallExternalAuthConsume) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	var body apps.ConsumeExternalAuthRequest
	if !decodeJSON(r, &body) {
		s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid JSON body", true)
		return
	}

	session, err := s.apps().ConsumeExternalAuthSession(r.Context(), principal.ID, r.PathValue("installIntentRef"), body.HandoffStateRef)
	if err != nil {
		if errors.Is(err, apps.ErrInvalidInstallIntent) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid external auth handoff session", true)
			return
		}
		if errors.Is(err, apps.ErrExternalAuthSessionNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "External auth handoff session not found", true)
			return
		}
		if errors.Is(err, apps.ErrExternalAuthSessionConsumed) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "External auth handoff session already consumed", true)
			return
		}
		if errors.Is(err, apps.ErrExternalAuthSessionExpired) {
			s.sendError(w, r, http.StatusGone, "GONE", "External auth handoff session expired", true)
			return
		}
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
...
func (s *Server) previewAppInstallIntentExternalAuthAuthorization(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallExternalAuthPreview) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}
	...
	preview, err := s.apps().PreviewExternalAuthAuthorization(r.Context(), principal.ID, r.PathValue("installIntentRef"), body.HandoffStateRef)
	if err != nil {
		if errors.Is(err, apps.ErrInvalidInstallIntent) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid external auth authorization preview", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotFound) || errors.Is(err, apps.ErrExternalAuthSessionNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "External auth handoff session not found", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotReady) {
	...
	s.sendJSON(w, r, http.StatusOK, envelope{
		Status: "success",
		Data: map[string]any{
			"externalAuthAuthorizationPreview": preview,
			"requestId":                        s.requestID(r),
		},
	})
}

func (s *Server) preflightAppInstallIntentExternalAuthCallback(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallExternalAuthCallback) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	var body apps.ExternalAuthCallbackPreflightRequest
	if !decodeStrictJSON(r, &body) {
		s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid JSON body", true)
		return
	}

	preflight, err := s.apps().PreflightExternalAuthCallback(r.Context(), principal.ID, r.PathValue("installIntentRef"), body)
	if err != nil {
		if errors.Is(err, apps.ErrInvalidExternalAuthCallbackPreflight) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid external auth callback preflight", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotFound) || errors.Is(err, apps.ErrExternalAuthSessionNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "External auth handoff session not found", true)
		...
		if errors.Is(err, apps.ErrExternalAuthCallbackPreflightConsumed) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "External auth callback preflight already recorded", true)
			return
		}
		if errors.Is(err, apps.ErrExternalAuthSessionExpired) {
			s.sendError(w, r, http.StatusGone, "GONE", "External auth handoff session expired", true)
	...
	s.sendJSON(w, r, http.StatusOK, envelope{
		Status: "success",
		Data: map[string]any{
			"externalAuthCallbackPreflight": preflight,
			"requestId":                     s.requestID(r),
		},
	})
}

func (s *Server) exchangeAppInstallIntentExternalAuthProvider(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallProviderExchange) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	var body apps.ExternalAuthProviderExchangeRequest
	if !decodeStrictJSON(r, &body) {
		s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid JSON body", true)
		return
	}

	exchange, err := s.apps().ExchangeExternalAuthProvider(r.Context(), principal.ID, r.PathValue("installIntentRef"), body)
	if err != nil {
		if errors.Is(err, apps.ErrInvalidExternalAuthProviderExchange) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid external auth provider exchange", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotFound) || errors.Is(err, apps.ErrExternalAuthCallbackPreflightNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "External auth callback preflight not found", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotReady) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "App install intent is not ready for external auth", true)
		...
		if errors.Is(err, apps.ErrExternalAuthProviderExchangeRecorded) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "External auth provider exchange already recorded", true)
			return
		}
		if errors.Is(err, apps.ErrAppNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "App not found", true)
	...
	s.sendJSON(w, r, http.StatusOK, envelope{
		Status: "success",
		Data: map[string]any{
			"externalAuthProviderExchange": exchange,
			"requestId":                    s.requestID(r),
		},
	})
}

func (s *Server) completeAppInstallIntent(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallComplete) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	var body apps.AppInstallCompletionRequest
	if !decodeStrictJSON(r, &body) {
		s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid JSON body", true)
		return
	}

	completion, err := s.apps().CompleteAppInstall(r.Context(), principal.ID, r.PathValue("installIntentRef"), body)
	if err != nil {
		if errors.Is(err, apps.ErrInvalidAppInstallCompletion) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid app install completion", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotFound) || errors.Is(err, apps.ErrExternalAuthProviderExchangeNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "External auth provider exchange not found", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotReady) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "App install intent is not ready for completion", true)
			return
		}
		if errors.Is(err, apps.ErrAppInstallCompletionRecorded) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "App install completion already recorded", true)
			return
		}
		if errors.Is(err, apps.ErrAppNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "App not found", true)
	...
	s.sendJSON(w, r, http.StatusOK, envelope{
		Status: "success",
		Data: map[string]any{
			"appInstallCompletion": completion,
			"requestId":            s.requestID(r),
		},
	})
}

func (s *Server) activateAppInstallIntent(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallActivate) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	var body apps.AppInstallationActivationRequest
	if !decodeStrictJSON(r, &body) {
		s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid JSON body", true)
		return
	}

	installation, err := s.apps().ActivateAppInstallation(r.Context(), principal.ID, r.PathValue("installIntentRef"), body)
	if err != nil {
		if errors.Is(err, apps.ErrInvalidAppInstallation) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid app installation", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotFound) || errors.Is(err, apps.ErrAppInstallCompletionNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "App install completion not found", true)
			return
		}
		if errors.Is(err, apps.ErrInstallIntentNotReady) || errors.Is(err, apps.ErrAppInstallCompletionNotReady) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "App install completion is not ready for activation", true)
			return
		}
		if errors.Is(err, apps.ErrAppInstallationRecorded) {
			s.sendError(w, r, http.StatusConflict, "CONFLICT", "App installation already recorded", true)
			return
		}
		if errors.Is(err, apps.ErrAppNotFound) {
			s.sendError(w, r, http.StatusNotFound, "NOT_FOUND", "App not found", true)
...
func (s *Server) readAppInstallIntentProgress(w http.ResponseWriter, r *http.Request) {
	principal, ok, err := s.authenticateAPIKey(r)
	if err != nil {
		s.sendError(w, r, http.StatusInternalServerError, "INTERNAL_SERVER_ERROR", "Internal server error", true)
		return
	...
	if !s.authorize(principal, authz.PolicyAppInstallProgressRead) {
		s.sendError(w, r, http.StatusForbidden, "FORBIDDEN", "Insufficient permissions", true)
		return
	}

	progress, err := s.apps().ReadInstallProgress(r.Context(), principal.ID, r.PathValue("installIntentRef"))
	if err != nil {
		if errors.Is(err, apps.ErrInvalidInstallIntent) {
			s.sendError(w, r, http.StatusBadRequest, "BAD_REQUEST", "Invalid app install intent", true)
			return
