...
The API listens on `http://localhost:8080`; Postgres is exposed on `localhost:54320` for local tooling. When `CALDIY_DATABASE_URL` is set, the backend runs embedded migrations and stores the accepted API-key principal, OAuth client metadata, and booking fixture/idempotency canaries in Postgres while preserving the same API response contract.
